...
rfcp /afs/cern.ch/cms/CAF/CMSALCA/ALCA_TRACKERALIGN/data/commonValidation/CMSSW_7_4_1/src/Alignment/OfflineValidation/test/forAPE/geometry_comp_test_new_v1_v2-2/usedConfiguration.ini /afs/cern.ch/cms/CAF/CMSALCA/ALCA_TRACKERALIGN/data/commonValidation/results/$USER/output_geometry_comparison/geometry_comp_test_new_v1_v2-2

if [[ $HOSTNAME = lxplus[0-9]*\.cern\.ch ]] # check for interactive mode
then
    mkdir -p /tmp/$USER/geometry_comp_test_new_v1_v2-2/2873530821
...
root_files=$(cmsLs -l /store/caf/user/$USER/AlignmentValidation/geometry_comp_test_new_v1_v2-2 | awk '{print $5}'              | grep ".root$" | grep -v "result.root$")
...
then
    ...
    root -x -q -b -l 'compareAlignments.cc++("root://eoscms.cern.ch//eos/cms/store/caf/user/ajkumar/AlignmentValidation/geometry_comp_test_new_v1_v2-2/comparedTracker_my_misalined_vs_my_IDEAL.root=my_misalined|4|1")'
    mv result.root GeometryComparison.Tracker_result.root
    cmsStage -f GeometryComparison.Tracker_result.root /store/caf/user/$USER/AlignmentValidation/geometry_comp_test_new_v1_v2-2
else
    echo "GeometryComparison.Tracker_result.root is up-to-date, no need to compare again."
    cmsStage -f /store/caf/user/$USER/AlignmentValidation/geometry_comp_test_new_v1_v2-2/GeometryComparison.Tracker_result.root .
fi
...
cd /afs/cern.ch/cms/CAF/CMSALCA/ALCA_TRACKERALIGN/data/commonValidation/CMSSW_7_4_1/src/Alignment/OfflineValidation/test/forAPE/geometry_comp_test_new_v1_v2-2
