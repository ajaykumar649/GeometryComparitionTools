...
void TrackerGeometryCompare::analyze(const edm::Event&, const edm::EventSetup& iSetup){
  ...
  if (firstEvent_) {
	...
	if (_writeToDB){
		...
		AlignmentErrorsExtended* myAlignmentErrors = currentTracker->alignmentErrors();
		...
		poolDbService->writeOne<AlignmentErrorsExtended>(&(*myAlignmentErrors), poolDbService->beginOfTime(), "TrackerAlignmentErrorExtendedRcd");
